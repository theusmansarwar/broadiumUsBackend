...
const CreateLeads = async (req, res) => {
  const { name, email, phone, service, business, query } = req.body;
  const missingFields = [];
  ...
  if (!service) missingFields.push({ name: "service", message: "Service field is required" });
  if (!business) missingFields.push({ name: "business", message: "Business field is required" });
  ...
  try {
    ...
    sendEmailToCompany({ email, name, phone, service, business, query }, res);
